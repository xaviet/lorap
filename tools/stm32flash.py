...
class Stm32bl():
  ...
  def _cmd_get(self):
    ...
    self.log(','.join('0x%02x'%(el, ) for el in self._allowed_commands), 'allowed_commands', level=1)
    return boot_version
  ...
  def _cmd_get_id(self):
    ...
    res = self._send_command(self.CMD_GET_ID, 4)
    if len(res) - 2 != res[0]:
  ...
  def write_file(self, address, file_name, verify=True):
    ...
    self.erase_blocks([i for i in range(size // 1024 + 1)])  
...
def main():
  ...
  parser.add_argument('-f', '--verify', action='store_true', help='Verify after writing', default=True)
